...
/**
 * data: { dateLabel, net, netLabel, shortfall, breakEven, rows:[[label,value]], disclaimer, url }
...
export function buildShareCanvas(data, colors) {
  ...
  ctx.fillText(data.netLabel, 64, 190);

  ctx.fillStyle = data.shortfall ? colors.warm : colors.ink;
  ctx.font = `700 104px ${sans}`;
  ctx.fillText(data.net, 60, 300);

  let y = 350;
  if (data.breakEven) {
    ctx.fillStyle = colors.warm;
    ctx.font = `700 32px ${sans}`;
    ctx.fillText(`手取りが0になる価格 ${data.breakEven}`, 64, y);
    y += 46;
  }
  ...
  ctx.fillText("住宅売却の手取り試算", 64, H - 48);
...
export function buildShareText(data) {
  return [
    `【住宅売却の手取り試算】${data.dateLabel} 時点`,
    ...data.rows.map(([label, value]) => `${label}：${value}`),
    `${data.netLabel}：${data.net}`,
    ...(data.breakEven ? [`手取りが0になる価格：${data.breakEven}`] : []),
    "",
    `※${data.disclaimer}`,
